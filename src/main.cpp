...
vtkSmartPointer<vtkPolyData> find_best_slice(
...
double compute_area_slice(vtkSmartPointer<vtkPolyData> slice,
    ...
    for (int icell = 0; icell < numcells; icell++) {
        ...
        area_cells[icell] =
            std::sqrt(s * (s - edge_sizes[0]) * (s - edge_sizes[1]) *
                      (s - edge_sizes[2]));
...
#pragma omp critical(status)
        ...
        for (int ipoint = 0; ipoint < num_point_arrays_slice; ipoint++) {
            ...
            if (starts_with(name, "pressure")) {
                double result[1];
                result[0] = integrate_on_slice(slice, numcells, area_cells,
        ...
        std::cout << "Completed slice " << my_slice << "/" << num_cl_points
                  << std::endl;
    }
    ...
    std::cout << "Time per slice: " << duration.count() / num_slices_processed
              << " s" << std::endl;
}
