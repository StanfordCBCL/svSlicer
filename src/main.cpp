 ...
 */
vtkSmartPointer<vtkPolyData> get_clean_slice(
    double position[3], vtkSmartPointer<vtkPolyData> slice) {
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3) {
    std::cerr << "Usage: svslicer [RESULT_FILE] [CENTERLINE_FILE] [OUTPUT_FILE]"
              << std::endl;
    return EXIT_FAILURE;
  }
  ...
  clean_up_point_data<vtkPolyData>(
      ...
      {"BifurcationId", "BifurcationIdTmp", "BranchId", "BranchIdTmp",
       "CenterlineId", "CenterlineSectionArea", "CenterlineSectionBifurcation",
       "CenterlineSectionClosed", "CenterlineSectionMaxSize",
       "CenterlineSectionMinSize", "CenterlineSectionNormal",
       "CenterlineSectionRemove", "CenterlineSectionShape", "GlobalNodeId",
...
#pragma omp critical(status)
    ...
    for (int point_array_idx = 0; point_array_idx < num_point_arrays_slice;
         point_array_idx++) {
      auto name =
          std::string(slice->GetPointData()->GetArrayName(point_array_idx));
      ...
      if (starts_with(name, "velocity")) {
        ...
        for (int j = 0; j < num_points_slice; j++) {
          double *vel =
              slice->GetPointData()->GetArray(point_array_idx)->GetTuple3(j);
          double curflux = 0.0;
          for (int k = 0; k < 3; k++) {
    ...
    std::cout << "Completed slice " << my_slice << "/" << num_centerline_points
              << std::endl;
  }
